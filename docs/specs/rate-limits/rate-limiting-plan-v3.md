# Rate-Limiting mit Supabase (Light) — Implementierungsplan v3
...
- Falls das bestehende Projekt noch auf dem alten Key-Modell läuft, funktioniert `service_role` genauso — die Migration auf den neuen Secret Key ist dann kein Blocker.
...
Bei ca. 60 Requests/Tag global und den üblichen Kosten pro Quiz-Generierung bleibt das im Bereich von rund 9 $ pro 30 Tage. Die harte Budgetgrenze beim KI-Anbieter fängt Abweichungen auf.
...
3. Secret Key bereithalten (neuer `sb_secret_...` oder bestehender `service_role`-Key).
...
4. Smoke-Test:
   ...
   - 6. Request vom selben Gerät → `429`
...
| 5 | 5. `generate`-Request (Endpunkt-Limit) | 429 |
| 6 | 3. `mixed`-Request (Endpunkt-Limit) | 429 |
| 7 | 6. Request gesamt am selben Tag (Gesamtlimit) | 429 |
...
1. **`@expo/app-integrity`:** Attestation-Token als zusätzlichen Guard vor dem Quota-Check. Kann die client-seitige UUID als Geräte-Identifier ablösen.
2. **Supabase Anonymous Auth:** JWT-basierte Identität nachrüsten, falls stärkere Zuordnung nötig wird. Die Quota-Tabelle bleibt, `device_id_hash` wird durch `user_id` ersetzt.
3. **Caching:** Vorab generierte Fragen für häufige Quiz-Kombinationen, um API-Kosten zu senken.
4. **Gewichtete Quotas:** Unterschiedliche Kosten pro Endpunkt, falls einer deutlich teurer wird.
5. **Atomare DB-Funktion (RPC):** Falls das Volumen deutlich steigt und Race Conditions relevant werden.
